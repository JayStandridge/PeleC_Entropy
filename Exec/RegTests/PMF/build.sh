...
if [ "${LMOD_SYSTEM_NAME}" == 'summit' ]; then
  GNUMAKE_ARGS="USE_CUDA=TRUE USE_MPI=TRUE PELE_USE_MAGMA=FALSE"
  SPACK_COMPILER="%gcc"
  SPACK_VARIANTS=""
  MACHINE="summit"
  ...
  GNUMAKE_ARGS="USE_HIP=TRUE USE_MPI=TRUE PELE_USE_MAGMA=FALSE"
  ...
  GNUMAKE_ARGS="COMP=intel USE_MPI=TRUE"
...
GNUMAKE_ARGS="USE_HDF5=TRUE USE_HDF5_ZFP=TRUE USE_ASCENT=FALSE USE_CONDUIT=FALSE HDF5_HOME=${HDF5_ROOT} H5Z_HOME=${H5Z_ROOT} ZFP_HOME=${ZFP_ROOT} ASCENT_DIR=${ASCENT_ROOT} CONDUIT_DIR=${CONDUIT_ROOT} ${GNUMAKE_ARGS}"
